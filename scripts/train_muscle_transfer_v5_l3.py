...
# -*- coding: utf-8 -*-
"""
肌肉特征迁移学习 V5 - L3椎体专注版本

核心思想：
1. 只关注L3椎体所在的切片，忽略其他部分
2. 空间自注意力：学习图像不同位置之间的相似性
3. 跨区域注意力：以已标注肌肉区域为参考，关注未标注但特征相似的区域
4. 位置编码：让网络感知空间位置信息

网络会学习到：某个位置的特征如果和已标注肌肉区域的特征相似，
那么它很可能也是肌肉区域。

关键改动：使用vertebrae_L3.nii.gz定位L3椎体，只保留L3区域的切片
...
# L3椎体定位文件
L3_VERTEBRA_FILE = 'vertebrae_L3.nii.gz'

# L3区域切片扩展（在L3椎体上下各扩展几个切片）
L3_SLICE_MARGIN = 2  # 上下各扩展2个切片
...
class MuscleRefineDataset(Dataset):
    """肌肉精修数据集 - L3专注版：只使用L3椎体所在的切片"""

    def __init__(self, subjects, target_shape=(256, 256), l3_only=True):
        self.items = []
        self.target_shape = target_shape
        self.muscle_files = KNOWN_MUSCLE_FILES
        self.non_muscle_files = NON_MUSCLE_FILES
        self.l3_only = l3_only

        # 预加载每个受试者的所有segmentation文件路径（用于后续加载）
        self.subject_seg_cache = {}

        print(f"构建数据集... (L3专注模式: {l3_only})")
        print(f"   已知肌肉文件: {len(self.muscle_files)} 个")
        print(f"   非肌肉排除文件: {len(self.non_muscle_files)} 个")
        print(f"   L3切片扩展margin: {L3_SLICE_MARGIN}")
        total_slices = 0
        kept_slices = 0
        subjects_with_l3 = 0
        subjects_without_l3 = 0
        ...
        for s in tqdm(subjects, desc="扫描受试者"):
            ...
            # L3椎体定位
            l3_path = os.path.join(seg_dir, L3_VERTEBRA_FILE)
            l3_slice_range = None
            
            if self.l3_only:
                if os.path.exists(l3_path):
                    l3_mask = nib.load(l3_path).get_fdata()
                    # 找到L3椎体存在的所有切片
                    l3_slices = []
                    for z in range(l3_mask.shape[2]):
                        if l3_mask[:, :, z].sum() > 0:
                            l3_slices.append(z)
                    
                    if l3_slices:
                        # 计算L3切片范围（加上margin）
                        l3_min = max(0, min(l3_slices) - L3_SLICE_MARGIN)
                        l3_max = min(depth - 1, max(l3_slices) + L3_SLICE_MARGIN)
                        l3_slice_range = (l3_min, l3_max)
                        subjects_with_l3 += 1
                    else:
                        subjects_without_l3 += 1
                        continue  # 跳过没有L3数据的受试者
                else:
                    subjects_without_l3 += 1
                    continue  # 跳过没有L3标注的受试者

            for z in range(depth):
                total_slices += 1

                if self.l3_only and l3_slice_range is not None:
                    # 只保留L3范围内的切片
                    if z < l3_slice_range[0] or z > l3_slice_range[1]:
                        continue
        ...
        print(f"   L3筛选统计:")
        print(f"      有L3标注的受试者: {subjects_with_l3}")
        print(f"      无L3标注的受试者: {subjects_without_l3} (已跳过)")
        print(f"      保留切片: {kept_slices}/{total_slices} ({100*kept_slices/max(total_slices,1):.1f}%)")
...
def visualize_predictions(model, dataloader, device, epoch, output_dir, num_samples=4):
    ...
    plt.suptitle(f'Epoch {epoch} - Attention-based Muscle Transfer V5 (L3 Focus)', fontsize=14, fontweight='bold')
    plt.tight_layout()
...
def main():
    print("=" * 60)
    print("肌肉特征迁移学习 V5 - L3椎体专注版本")
    print("=" * 60)
    print("新特性：")
    print("  - 只关注L3椎体所在的切片")
    print("  - 使用vertebrae_L3.nii.gz定位L3区域")
    print("  - 排除非肌肉区域（器官、骨骼、血管等）")
    print("  - 只在未标注区域寻找相似肌肉")
    print("=" * 60)

    DATA_ROOT = '/local/hzhang02/data'
    OUTPUT_DIR = '/local/hzhang02/data/dataset/outputs_v5'
    ...
    wandb.init(
        ...
        name=f"transfer-v5-l3-{time.strftime('%Y%m%d-%H%M%S')}",
        config={
            "version": "v5-l3-focused",
            "description": "只关注L3椎体区域的切片，排除所有非肌肉区域",
            ...
            "l3_only": L3_ONLY,
            "l3_slice_margin": L3_SLICE_MARGIN,
            "l3_vertebra_file": L3_VERTEBRA_FILE,
    ...
    train_ds = MuscleRefineDataset(train_subs, TARGET_SHAPE, l3_only=L3_ONLY)
    val_ds = MuscleRefineDataset(val_subs, TARGET_SHAPE, l3_only=L3_ONLY)
